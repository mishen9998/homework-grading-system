  ...
  <div class="student-messages">
        ...
        <div v-else>
          ...
          >
            <div class="message-icon">
              <span v-if="message.message_type === 'assignment_reminder'">🔔</span>
              <span v-else-if="message.message_type === 'assignment'">📚</span>
              <span v-else-if="message.message_type === 'grade'">✅</span>
              <span v-else>📢</span>
            </div>
    ...
    <div v-if="activeTab === 'friends'" class="friends-content">
      <div class="my-code-card">
        ...
        <p class="code-hint">分享此代码给好友，让他们添加你</p>
      </div>
...
const handleMessageClick = async (message) => {
  if (!message.is_read) {
    try {
      await assignmentAPI.markMessageRead(message.id)
...
const loadMessages = async () => {
  loading.value = true
  try {
    const res = await assignmentAPI.getMessages()
