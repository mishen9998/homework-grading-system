...
class DesktopApp:
    def __init__(self, window, auto_start=True, auto_open=True):
        ...
        ttk.Button(footer, text='使用说明', command=lambda: self.open_path(ROOT / 'docs/guides/桌面助手使用说明.md')).pack(side='left', padx=8)
        ttk.Button(footer, text='数据库迁移指南', command=lambda: self.open_path(ROOT / 'docs/guides/数据库迁移指南.md')).pack(side='left')
